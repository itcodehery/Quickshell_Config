...
Item {
    ...
    implicitWidth: root.modMpris
        ? (active
            ? ((fullMode ? fullRow.implicitWidth : (islandMode ? islandContent.implicitWidth : defaultRow.implicitWidth)) + 18)
    ...
    Row {
        id: defaultRow
        visible: rootMod.active && !rootMod.fullMode && !rootMod.islandMode
        anchors.centerIn: parent
        spacing: 4
    ...
    // ── ISLAND: album art + title + eq ──────────────────────────────
    Item {
        id: islandRow
        visible: rootMod.active && rootMod.islandMode
        anchors.centerIn: parent
        ...
        Row {
            ...
            ClippingRectangle {
                ...
                anchors.verticalCenter: parent.verticalCenter
                color: Qt.rgba(rootMod.accentColor.r, rootMod.accentColor.g, rootMod.accentColor.b, 0.2)
                Image {
                    id: islandArt
                    anchors.fill: parent
            ...
            Canvas {
                ...
                Connections {
                    target: rootMod
                    function onBarH1Changed() { islandEq.requestPaint() }
                    function onBarH2Changed() { islandEq.requestPaint() }
                    function onBarH3Changed() { islandEq.requestPaint() }
                }
                Component.onCompleted: requestPaint()
            }
        }
